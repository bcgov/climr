...
We begin by extracting all the points of our input raster, adding elevation
values to each (if not in the raster already) and passing a table of point
locations to `climr_downscale()`.
...
Now we simply need to use our original raster as a template to make rasters of downscaled climate values.
