...
###Using the `climr_downscale()` function

The basic use of `climr` is to pass a `data.table` of point coordinates,
IDs and elevation to `climr_downscale()` and select the type of climate
...
These functions can be used as arguments in `climr_downscale()`. For example, `vars = list_variables()` could be specified to obtain data for all available variables, or `gcm_models = list_gcm()[c(1,4,5,6,7,10,11,12)]` could be used to specify the eight-gcm ensemble recommended by Mahony et al. (2022). 
...
my_grid <- as.data.frame(dem_vancouver, cells=T, xy=T)
...
values(my_clim) <- NA # remove the elevation values from the 
