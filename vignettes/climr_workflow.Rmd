...
We will chose the only available historic period (2001-2020), 2021-2040 as the future period, the 'EC-Earth3' GCM and the SSP 2.45 scenario. MAT, PPT and RH will be selected as output variables.
...
[ClimateNA](climatena.ca). 'normal_composite' corresponds to a composite of 
...
(which defaults to using the highest resolution data available for the points selected). 
...
Also, future projections were obtained for two runs of EC-Earth3 ('r1i1p1f1' and 
...
We may also want yearly climate projections. In this case, we want the yearly values of MAT and PPT for 2001-2015 and 2021:2040, using the same GCM, SSP and number of model
runs. Notice how some of the data doesn't need to be downloaded again, and was retrieved from cache.

<!-- missing data for years 2016-2020 -->
<!-- wclimr does bias correction between observed and modelled historic valeus - link to methods vignette -->

<!-- don't show two vars in 2 yaxes-->
...
To plot the time series, we will filter the data to a single model run (i.e. in this
case discard the ensemble means). Note that we plot both the observed and the projected
historical climate values.

```{r, fig.cap = "Time series outputs from `climr_downscale`. The lines show mean annual temperature (MAT() and total annual precipitation (PPT) values averaged across model runs."}
## here -- server went down.
ds_out_ts[is.na(GCM), GCM := "Historic"]
ds_out_ts[!grepl("ensemble", RUN)]

## make groups so that mising data is not shown as a "line connection"
groups <- data.table(PERIOD = unique(ds_out_ts2$PERIOD))
groups[, idx := c(1, diff(PERIOD))]
i2 <- c(1, which(groups$idx != 1), nrow(groups) + 1)
groups[, grp := rep(1:length(diff(i2)), diff(i2))]
ds_out_ts2 <- groups[, .(PERIOD, grp)][ds_out_ts2, on = "PERIOD"]
ds_out_ts2[, grp := paste(grp, variable, sep = "_")]

yrbreaks <- c(
  min(ds_out_ts2$PERIOD),
...
if (require(ggplot2)) {
  ggplot(
    ...
    scale_color_manual(values = c("MAT" = "#D6604D", "PPT" = "#4393C3")) +
    theme_light() +
    theme(axis.text.x = element_text(angle = 45, vjust = 0.5)) +
    labs(x = "Year", y = expression("MAT"~~(degree*"C")), col = "")
}
```
...
of points and plot the values of a chosen climate variable from the list passed to 
`climr_downscale(..., vars)`.
