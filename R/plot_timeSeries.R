...
#' @param legend_pos character. Position of the legend. Viable options are "bottomright",
...
#'   plot_timeSeries(my_data, gcms = list_gcms()[c(7, 13)], pal = "gcms",
#'                   ssps = list_ssps()[2], showmean = FALSE, compile = FALSE,
#'                   simplify = FALSE, endlabel = "gcms", mar = c(3, 3, 0.1, 6),
#'                   showObserved = FALSE)
#'
...
plot_timeSeries <- function(
  ...
  if (!requireNamespace("scales", quietly = TRUE)) {
        ...
        for (scenario in scenarios.selected[order(c(1, 4, 5, 3, 2)[which(scenarios %in% scenarios.selected)])]) {
          ...
          if (showmean) {
            if (simplify) {
              ...
              lines(x = get(paste("x", scenario, sep = ".")), y = get(paste("ensmean", scenario, sep = ".")), col = colSelect(scenario, gcms), lwd = 2)
            }
          }
    ...
    if (showObserved) {
      ...
      legend(legend_pos,
        title = "", legend = c("Observed (ClimateNA)", "Observed (CRU/GPCC)", "Observed (ERA5)", legend.GCM)[s], bty = "n",
        lty = c(1, 1, 1, 1)[s],
    ...
    if (pal == "gcms") {
      ...
      legend(c("top", "bottom")[if (length(grep("top", legend_pos)) == 1) 1 else 2],
