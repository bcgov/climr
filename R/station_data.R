...
#' @param dataset Character. Which station dataset to use? Currently AHCCD is the only option.
...
#' @noRd
#' @export
get_station_data.data.frame <- function(xyz, dataset = "AHCCD", k = 1, date_range = c("2001-01-01", "2005-12-31"),
                                    vars = sort(sprintf(c("PPT_%02d", "Tmax_%02d", "Tmin_%02d"), sort(rep(1:12, 3)))), 
                                    only_complete = FALSE) {
  if(only_complete){
    qry <- glue("SELECT DISTINCT id
            FROM station_data
            WHERE make_date(year, month, 1) BETWEEN DATE '{date_range[1]}' AND DATE '{date_range[2]}'
            GROUP BY id
    ...
    st_meta <- db_safe_query(glue("SELECT id, lon, lat FROM station_meta WHERE id IN ('{c_ids}')"))
  } else {
    st_meta <- db_safe_query("SELECT id, lon, lat FROM station_meta")
  }
  ...
  st_dat <- db_safe_query(glue("SELECT s.id, m.lat, year, month, tmin, tmax, s.pr FROM 
                                        station_data s
                                        JOIN station_meta m USING (id)
                                        WHERE s.id IN ('{st_ids}') AND
                                        make_date(year, month, 1) BETWEEN DATE '{date_range[1]}' AND DATE '{date_range[2]}'"))
  setDT(st_dat)
  st_dat[, month := sprintf("%02d",month)]
  setnames(st_dat, old = c("year", "tmin","tmax","pr"), new = c("PERIOD", "Tmin","Tmax", "PPT"))
...
get_station_data.numeric <- function(bbox, dataset = "AHCCD", k = 1, date_range = c("2001-01-01", "2005-12-31"),
  ...
  if(only_complete){
    ...
    SELECT f.id, f.lat, f.lon, f.year, f.month, f.tmin, f.tmax, f.pr
    FROM filtered f
