...
for(e in 1:length(elements)){
  ...
  for(m in 1:12){
    ...
    png(filename=paste("//objectstore2.nrs.bcgov/ffec/Climatologies/climr_mosaic/maps/climr_mosaic_", period, "_", elements[e], monthcodes[m], ".png", sep=""), type="cairo", units="in", width=9.5, height=7.8, pointsize=12, res=700)
    par(mfrow=c(1,1), mar=c(0,0,0,0))
    plot(comp, col=ColScheme, breaks=breaks, legend=F, main="", axes=F, maxcell=ncell(comp), mar=NA)
    legend_ramp(comp, title = paste("1981-2010", month.name[m], element.names[e]), ColScheme = ColScheme, breaks = breaks, pos=c(0.2, 0.23, 0.1, 0.5), log = if(e==3) 2 else NULL, horizontal = FALSE, title.height = if(e==3) 1 else 2)
    dev.off()
